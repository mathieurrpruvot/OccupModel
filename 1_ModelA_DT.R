...
simulate_landscape <- function(forest_prop,smooth_strength){
    ...
    threshold <- quantile(terra::values(r_smooth), probs = 1 - forest_prop)     ## DT
...
##
## DT: this section below "10(B) SIMULATION LOOP" is new
##
############################################################
## 10(B). SIMULATION LOOP
############################################################
...
simData <- function() {
    ...
    detec <- simulate_detection(z, prop_cell_scout_gap, prop_scout_replicate,
                                prop_cell_drone_gap, prop_drone_replicate,
                                pWeekActive, siteActiveProb, seasonGapProb, maxGapLength, siteWideWeekMiss, ViewMaps)
    c(detec, land)
}
...
data <- simData()
dataVars <- c('yScout', 'yDrone', 'yCam')
for(var in dataVars)   data[[var]] <- array(0, dim(data[[var]]))

Rmodel <- nimbleModel(code, constants, data, inits)

monitors <- c('beta0', 'beta1', 'beta2', 'alpha0', 'alphaNbr', 'alpha2', 'alpha3', 'alphaControl',
              'delta0', 'delta1', 'theta0', 'theta_adequate', 'theta_good',
              'eta0', 'eta_adequate', 'eta_good', 'phi0', 'phi_old', 'phi_fresh')
conf <- configureMCMC(Rmodel, monitors = monitors)
nInitialSamplers <- length(conf$samplerConfs)
Rmodel$resetData()
for(var in dataVars)   conf$addSampler(var, expandTarget = TRUE, default = TRUE, print = FALSE)
samplerTargets <- sapply(conf$samplerConfs, function(x) x$target)
...
for(sim in 1:nSim) {
    ...
    ppNodes <- Cmodel$getNodeNames(predictiveOnly = TRUE)
    if(!all(ppNodes %in% samplerTargets))   stop()
    ppSamplerIndices <- sort(match(ppNodes, samplerTargets))
    newExecutionOrder <- c(1:nInitialSamplers, ppSamplerIndices)
    newExecutionOrder_touse <- c(newExecutionOrder, 0, 0)    ## pad with two 0's
    Cmcmc$samplerExecutionOrderFromConfPlusTwoZeros <- newExecutionOrder_touse
...
print(time_taken) #Time difference of 1.523925 hours for 1000 burnin, 2000 iteration, 3 sim
