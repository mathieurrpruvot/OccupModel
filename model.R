...
#   - need to make detection proba more realistic (lower)
#   - need to make colonization dependent on control activities, to limit the boom-bust dynamics on longer time scales
...
control   <- matrix(rbinom(N*T,1,0.9), N, T)
...
inits <- list(
  ...
  alpha0=0,alphaNbr=0,alpha2=0,alpha3=0,
  delta0=0,delta1=0,
  theta0=0,eta0=0,phi0=0
)
...
library(MCMCvis)
MCMCtrace(samples[,c(1:5)],open_pdf = F)
MCMCsummary(samples[,c(1:5)])
MCMCplot(samples)
